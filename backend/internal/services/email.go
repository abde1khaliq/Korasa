...
func SendPasswordResetCode(to string, code string) error {
	apiKey := config.App.ResendAPIKey
	if apiKey == "" {
		return fmt.Errorf("RESEND_API_KEY not configured")
	}
	...
	htmlContent := fmt.Sprintf(`
		...
		<body>
			<div class="wrapper">
				...
				<p class="body-text">Use this code to reset your password for %s.</p>
				<div class="code-box"><span class="code">%s</span></div>
				<p class="footnote">This code expires in 10 minutes. If you didn't request this, you can safely ignore this email.</p>
				<div class="divider"></div>
				<p class="legal">© %d Korasa. All rights reserved.</p>
	...
	params := &resend.SendEmailRequest{
		From:    "noreply@korasa.study",
		To:      []string{to},
		Subject: "Your Korasa password reset code",
		Html:    htmlContent,
	}
	...
	log.Printf("Password reset code sent to %s. Email ID: %s", to, sent.Id)
	return nil
}
