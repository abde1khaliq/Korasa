...
export function ExamsEmptyState({ onCreateClick }: { onCreateClick: () => void }) {
  const ink = useThemeColor("#F1EFEC", "#2B2724");
  return (
    <View className="flex-1">
      <View className="px-6 pt-8">
        <Text className="font-display text-[48px] leading-[52px] text-ink">Exams</Text>
        <Text className="mt-3 text-[17px] text-ink-soft">Test yourself on what you've saved.</Text>
      </View>
      <View className="flex-1 items-center justify-center px-6 pb-16">
        <View
          className="items-center justify-center rounded-full"
          style={{ width: 96, height: 96, backgroundColor: "rgba(156,144,134,0.1)" }}
        >
