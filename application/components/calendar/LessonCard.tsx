...
import { formatTimeRange, getDayName, getLessonStatusForDate } from "@/lib/lessonUtils";
...
export function LessonCard({ lesson, targetDate = new Date(), onEdit, onDelete }: LessonCardProps) {
  ...
  const handleDelete = () => {
    Alert.alert(
      "Delete Lesson",
      `Are you sure you want to delete the recurring "${lesson.title}" on ${getDayName(lesson.day_of_week)}s?`,
      [
  ...
  return (
    <View className="mb-3 rounded-2xl border border-rule bg-paper-card p-4">
      {/* Top row: Status, Day, Subject, Actions */}
      ...
      {/* Title */}
      <Text className="mt-2.5 text-[17px] font-semibold text-ink" numberOfLines={2}>
        {lesson.title}
      </Text>

      {/* Description */}
      {lesson.description ? (
        <Text className="mt-1 text-[13px] text-ink-soft" numberOfLines={2}>
          {lesson.description}
        </Text>
      ) : null}
