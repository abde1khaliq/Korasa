...
export function Onboarding() {
  ...
  return (
    <View className="flex-1">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ width }} className="flex-1 items-center justify-center px-10">
            <View
              className="items-center justify-center rounded-full border border-rule bg-paper-card"
              style={{ width: 140, height: 140 }}
            >
              <item.Icon size={56} color="#A8703F" strokeWidth={1.5} />
            </View>
      ...
      <View className="flex-row items-center justify-center px-6 pb-4" style={{ gap: 8 }}>
        {onboardingSlides.map((slide, i) => (
          <View
            key={slide.key}
            className="rounded-full bg-onyx"
            style={{ width: i === index ? 20 : 6, height: 6, opacity: i === index ? 1 : 0.25 }}
          />
        ))}
      </View>
